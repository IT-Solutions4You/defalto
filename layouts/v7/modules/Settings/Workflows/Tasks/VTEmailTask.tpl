	...
	<div id="VtEmailTaskContainer">
		<div class="row">
            ...
            <div class="row from-group">
                ...
                {/if}
                {if PrintTemplates_Merge_Model::isInventoryModule($SOURCE_MODULE)}
                    {assign var=COUNTER value=1}
                    <div class="col-sm-6 col-xs-6">
                        <div class="row">
                            <div style="margin-top: 7px" class="col-sm-3 col-xs-3">{vtranslate('LBL_ADD_TEMPLATE',$QUALIFIED_MODULE)}</div>&nbsp;&nbsp;
                            <div class="col-sm-8 col-xs-8">
                                <select style="min-width: 250px" id="task-templates" class="select2" data-placeholder={vtranslate('LBL_SELECT_OPTIONS',$QUALIFIED_MODULE)}>
                                    <option></option>
                                    {foreach from=$TEMPLATE_VARIABLES item=TEMPLATE_VARIABLE_KEY key=TEMPLATE_VARIABLE_VALUE}
                                        <option value='{$TEMPLATE_VARIABLE_VALUE}'>{$TEMPLATE_VARIABLE_KEY}</option>
                                    {/foreach}
                                </select>
                            </div>
                        </div>
            ...
            <div class="row form-group">
                {if Vtiger_Module_Model::isPrintTemplateEnabled($SOURCE_MODULE)}
                    {assign var = PDF_TEMPLATES value = PrintTemplates_Record_Model::getAllInstancesByRelatedModule($SOURCE_MODULE)}
                    {if count($PDF_TEMPLATES) ne 0}
                        <div class="col-sm-6 col-xs-6">
                            <div class="row">
                                <div style="margin-top: 7px; padding-right: 0px;" class="col-sm-2 col-xs-2">{vtranslate('LBL_ADD_PDF',$QUALIFIED_MODULE)}</div>
                                <div class="col-sm-1 col-xs-1" style="margin-top: 7px;">
                                    <input type="checkbox" name="pdf" {if $TASK_OBJECT->pdf eq 'on'} checked="checked" {/if}/>
                                </div>
                                {assign var = PDF_MODULE_NAME value = 'PrintTemplates'}
                                <div class="col-sm-8 col-xs-8">
                                    <div class="row">
                                        <div class="col-sm-9 col-xs-9">
                                            <select style="min-width: 250px" id="pdf-templates" name="pdfTemplateId" class="select2">
                                                {foreach from=$PDF_TEMPLATES item=TPL}
                                                    <option value="{$TPL->get('id')}" {if $TASK_OBJECT->pdfTemplateId eq $TPL->get('id')} selected="selected" {/if}>
                                                        {vtranslate($TPL->get('name'),$PDF_MODULE_NAME)}
                                                    </option>
                                                {/foreach}
                                            </select>
                                        </div>
				...
				<div class="col-sm-5 col-xs-5">
                    <div class="row">
                        <div style="margin-top: 7px" class="col-sm-4 col-xs-4">{vtranslate('LBL_INCLUDE_SIGNATURE',$QUALIFIED_MODULE)}</div>&nbsp;&nbsp;
                        <div class="col-sm-5 col-xs-5" style="margin-top: 7px">
                            <input class="inputElement" type="hidden" name="signature" value="No" checked="checked" />
                            <input class="inputElement" type="checkbox" name="signature" value="Yes" {if $TASK_OBJECT->signature eq 'Yes' || empty($TASK_OBJECT->signature)} checked="checked" {/if} id="signature"/>
                            &nbsp;<i class="icon-question-sign" style="vertical-align:middle;" id="signaturePopover" data-toggle="tooltip" data-placement="right" data-original-title="{vtranslate('LBL_INCLUDE_SIGNATURE',$QUALIFIED_MODULE)}" data-content="{vtranslate('LBL_INCLUDE_SIGNATURE_INFO',$QUALIFIED_MODULE)}"></i>
                        </div>	
                    </div>
				</div>
            </div>
