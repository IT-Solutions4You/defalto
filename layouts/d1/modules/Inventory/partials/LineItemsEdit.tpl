...
<div name='editContent'>
	...
	{if $BLOCK_FIELDS|@count gt 0}
		...
		<div class="fieldBlockContainer container-fluid px-4">
			<div class="row pb-3">
				<div class="col-sm-3 d-flex justify-content-end align-items-center">
					{if $LINEITEM_FIELDS['region_id'] && $LINEITEM_FIELDS['region_id']->isEditable()}
						<div class="input-group">
							...
							<select class="select2 form-control" id="region_id" name="region_id">
							...
							<div class="input-group-text">
								<a class="fa fa-wrench hidden-xs" href="index.php?module=Vtiger&parent=Settings&view=TaxIndex" target="_blank"></a>
							</div>
						</div>
				...
				<div class="col-sm-3 d-flex justify-content-end align-items-center">
					<div class="input-group">
						...
						{if $SELECTED_CURRENCY eq ''}
							...
							{/foreach}
						{/if}
						<select class="select2 form-control" id="currency_id" name="currency_id">
				...
				<div class="col-sm-3 d-flex justify-content-end align-items-center">
					<div class="input-group">
						...
						<select class="select2 lineItemTax form-control" id="taxtype" name="taxtype">
				...
				<table class="table table-borderless" id="lineItemTab">
					<tr class="border-bottom">
						...
						{if isset($PURCHASE_COST_EDITABLE)}
							<td>
								<strong class="text-end">{vtranslate({$LINEITEM_FIELDS['purchase_cost']->get('label')},$MODULE)}</strong>
							</td>
						{/if}
						...
						{/if}
						<td class="text-end">
							<strong>{vtranslate('LBL_TOTAL',$MODULE)}</strong>
						</td>
						{if isset($MARGIN_EDITABLE) && isset($PURCHASE_COST_EDITABLE)}
							<td class="text-end">
								<strong>{vtranslate({$LINEITEM_FIELDS['margin']->get('label')},$MODULE)}</strong>
							</td>
						{/if}
						<td class="text-end">
							<strong>{vtranslate('LBL_NET_PRICE',$MODULE)}</strong>
						</td>
					</tr>
					...
					{if $RELATED_PRODUCTS && php7_count($RELATED_PRODUCTS) eq 0 and ($PRODUCT_ACTIVE eq 'true' || $SERVICE_ACTIVE eq 'true')}
						<tr id="row1" class="border-bottom lineItemRow" data-row-num="1">
				...
				{if $PRODUCT_ACTIVE eq 'true' && $SERVICE_ACTIVE eq 'true'}
					<div class="btn-toolbar">
						<span class="btn-group me-2">
							<button type="button" class="btn btn-outline-secondary" id="addProduct" data-module-name="Products" >
								<i class="fa fa-plus"></i>
								<strong class="ms-2">{vtranslate('LBL_ADD_PRODUCT',$MODULE)}</strong>
							</button>
						</span>
						<span class="btn-group">
							<button type="button" class="btn btn-outline-secondary" id="addService" data-module-name="Services" >
								<i class="fa fa-plus"></i>
								<strong class="ms-2">{vtranslate('LBL_ADD_SERVICE',$MODULE)}</strong>
							</button>
						</span>
					</div>
		...
		<div class="fieldBlockContainer px-4 py-3">
			<table class="table table-borderless blockContainer lineItemTable" id="lineItemResult">
				<tr class="border-bottom">
					<td class="text-end">
					...
					<tr class="border-bottom">
						<td class="w-75 text-end">
							<div class="position-relative">
								<a href="javascript:void(0)" id="finalDiscount">
									<strong class="me-2">{vtranslate('LBL_OVERALL_DISCOUNT',$MODULE)}</strong>
									<span class="me-2">
										<span id="overallDiscount">
											...
											{elseif $DISCOUNT_AMOUNT_EDITABLE && $FINAL.discount_type_final eq 'amount'}
									...
									<table class="table table-borderless popupTable m-0">
										...
										<tr class="border-bottom">
											<td class="text-nowrap p-3">
												<label>
													<input type="radio" name="discount_final" class="finalDiscounts" data-discount-type="zero" {if $DISCOUNT_TYPE_FINAL eq 'zero'}checked{/if} />
													<span class="ms-2">{vtranslate('LBL_ZERO_DISCOUNT',$MODULE)}</span>
												</label>
											</td>
											<td class="lineOnTop">
										...
										{if $DISCOUNT_PERCENT_EDITABLE}
											<tr class="border-bottom">
												<td class="text-nowrap p-3">
													<label>
														<input type="radio" name="discount_final" class="finalDiscounts" data-discount-type="percentage" {if $DISCOUNT_TYPE_FINAL eq 'percentage'}checked{/if} />
										...
										{if $DISCOUNT_AMOUNT_EDITABLE}
											<tr class="border-bottom">
												<td class="text-nowrap p-3">
													<label>
														<input type="radio" name="discount_final" class="finalDiscounts" data-discount-type="amount" {if $DISCOUNT_TYPE_FINAL eq 'amount'}checked{/if} />
														<span class="ms-2">{vtranslate('LBL_DIRECT_PRICE_REDUCTION',$MODULE)}</span>
													</label>
												</td>
												<td>
				...
				{if $SH_PERCENT_EDITABLE}
					...
					<tr class="border-bottom">
						<td class="w-75 text-end">
							<div class="position-relative">
								<a href="javascript:void(0)" id="charges">
									<strong class="me-2">{vtranslate('LBL_CHARGES',$MODULE)}</strong>
									<span>(+)</span>
								</a>
									...
									<table class="table table-borderless popupTable m-0">
										{foreach key=CHARGE_ID item=CHARGE_MODEL from=$INVENTORY_CHARGES}
											<tr class="border-bottom">
												...
												<td class="lineOnTop chargeName p-3" data-charge-id="{$CHARGE_ID}">{$CHARGE_MODEL->getName()}</td>
												<td class="lineOnTop text-end">
													{if $CHARGE_MODEL->get('format') eq 'Percent'}
														<input type="text" class="form-control chargePercent" size="5" data-rule-positive=true data-rule-inventory_percentage=true name="charges[{$CHARGE_ID}][percent]" value="{if $CHARGE_PERCENT}{$CHARGE_PERCENT}{else if $RECORD_ID}0{else}{$CHARGE_MODEL->getValue()}{/if}" />&nbsp;%
													{/if}
												</td>
												<td class="lineOnTop text-end">
						...
						<td>
							<input type="hidden" class="form-control lineItemInputBox" id="chargesTotal" name="shipping_handling_charge" value="{if $FINAL.shipping_handling_charge}{$FINAL.shipping_handling_charge}{else}0{/if}" />
				...
				<tr class="border-bottom">
					...
					<td class="text-end">
						...
						<span id="preTaxTotal">{if $PRE_TAX_TOTAL}{$PRE_TAX_TOTAL}{else}0{/if}</span>
						<input type="hidden" id="pre_tax_total" name="pre_tax_total" value="{if $PRE_TAX_TOTAL}{$PRE_TAX_TOTAL}{else}0{/if}"/>
					</td>
				</tr>
				<!-- Group Tax - starts -->

				<tr id="group_tax_row" class="border-bottom align-top {if $IS_INDIVIDUAL_TAX_TYPE}opacity-0{/if}">
					<td class="w-75 text-end">
						<div class="position-relative">
							<a href="javascript:void(0)" id="finalTax">
							...
							<div class="hide finalTaxUI validCheck" id="group_tax_div">
								...
								<table class="table table-borderless popupTable m-0">
									{foreach item=tax_detail name=group_tax_loop key=loop_count from=$TAXES}
										<tr>
											<td class="lineOnTop p-3">{$tax_detail.taxlabel}</td>
											<td class="lineOnTop">
												<div class="input-group">
											...
											<td class="lineOnTop text-end">
												<input type="text" size="6" name="{$tax_detail.taxname}_group_amount" id="group_tax_amount{$smarty.foreach.group_tax_loop.iteration}" style="cursor:pointer;" value="{$tax_detail.amount}" readonly class="form-control groupTaxTotal" />
											</td>
										</tr>
									{/foreach}
					...
					<td class="text-end">
						<span id="tax_final" class="tax_final">{if $FINAL.tax_totalamount}{$FINAL.tax_totalamount}{else}0{/if}</span>
					</td>
				</tr>
				<!-- Group Tax - ends -->
				{if $SH_PERCENT_EDITABLE}
					<tr class="border-bottom">
						<td class="w-75 text-end">
							<div class="position-relative">
								<a href="javascript:void(0)" id="chargeTaxes">
									<strong class="me-2">{vtranslate('LBL_TAXES_ON_CHARGES',$MODULE)}</strong>
									<span>(+)</span>
								</a>
											...
											{foreach key=CHARGE_ID item=CHARGE_MODEL from=$INVENTORY_CHARGES}
												{foreach key=CHARGE_TAX_ID item=CHARGE_TAX_MODEL from=$RECORD->getChargeTaxModelsList($CHARGE_ID)}
													...
													<tr class="border-bottom">
														...
														<td class="lineOnTop p-3 text-nowrap">{$CHARGE_MODEL->getName()} - {$CHARGE_TAX_MODEL->getName()}</td>
														...
														<td class="lineOnTop text-end w-25">
															<input type="text" class="form-control chargeTaxValue chargeTax{$CHARGE_ID}{$CHARGE_TAX_ID}" size="5" value="0" readonly />
														</td>
													</tr>
												{/foreach}
											{/foreach}
						...
						<td class="text-end">
							...
							<span id="chargeTaxTotal">{if $FINAL.shtax_totalamount}{$FINAL.shtax_totalamount}{else}0{/if}</span>
						</td>
					</tr>
					<tr class="border-bottom">
						<td class="w-75 text-end">
							<div class="position-relative">
								<a href="javascript:void(0)" id="deductTaxes">
									<strong class="me-2">{vtranslate('LBL_DEDUCTED_TAXES',$MODULE)}</strong>
									<span>(-)</span>
								</a>
						...
						<td class="text-end">
							<span id="deductTaxesTotalAmount">{if $FINAL.deductTaxesTotalAmount}{$FINAL.deductTaxesTotalAmount}{else}0{/if}</span>
						</td>
					</tr>
				{/if}
				...
				<tr class="border-bottom align-top">
					<td class="w-75 text-end">
						<div>
							...
							<label class="ms-3">
								<input type="radio" name="adjustmentType" option value="+" {if $FINAL.adjustment gte 0}checked{/if}>
								<span class="ms-2">{vtranslate('LBL_ADD',$MODULE)}</span>
							</label>
							<label class="ms-3">
								<input type="radio" name="adjustmentType" option value="-" {if $FINAL.adjustment lt 0}checked{/if}>
								<span class="ms-2">{vtranslate('LBL_DEDUCT',$MODULE)}</span>
							</label>
						</div>
					</td>
					<td>
