...
Vtiger.Class('Settings_LayoutEditor_Js', {
    ...
    showFieldEditModel: function (data, blockId, fieldContainer) {
        ...
        data.find('[name="pickListValues"]').on('change select2:select', function (e) {
            ...
            let defaultOptions = defaultField.find('option').removeClass('doestNotExists').addClass('doesNotExists'),
                emptyOption = defaultOptions.filter('[value=" "]').removeClass('doesNotExists'),
                fieldOptions = element.select2('val'),
                newOptions = '';

            for (let i in fieldOptions) {
                let fieldValue = fieldOptions[i],
                    specialChars = /[<\>\"\,\[\]\{\}]/;

                if (specialChars.test(fieldValue)) {
                    let message = app.vtranslate('JS_SPECIAL_CHARACTERS') + " < > \" , [ ] { } " + app.vtranslate('JS_NOT_ALLOWED');
                ...
                let fieldValueOption = defaultOptions.filter('[value="' + fieldValue + '"]');
        ...
        let isEditMode = false,
            formFieldId = form.find('input[name="fieldid"]').val();
            ...
            submitHandler: function (form) {
                ...
                if (!isEditMode) {
                    ...
                    if (fieldTypeValue === 'Picklist' || fieldTypeValue === 'MultiSelectCombo') {
                        let pickListValueElement = jQuery('#picklistUi', form),
                            pickLisValues = pickListValueElement.select2('val'),
                            select2Element = app.getSelect2ElementFromSelect(pickListValueElement),
                            pickListValuesArray = pickLisValues,
    ...
    registerAddCustomFieldEvent: function () {
        ...
        contents.find('.addCustomField').click(function (e, params) {
            ...
            let blockElement = jQuery(e.currentTarget).closest('.editFieldsTable'),
                blockId = blockElement.data('blockId'),
                addFieldContainer = contents.find('.createFieldModal').clone(true, true);
    ...
    registerFieldTypeHide: function (form) {
        form.find('.decimalsupported').addClass('hide');
        form.find('.preDefinedValueExists').addClass('hide');
        form.find('.picklistOption').addClass('hide');
        form.find('.relationModules').addClass('hide');
    },
    /**
    ...
    showCustomField: function (result) {
        let thisInstance = this,
            contents = jQuery('#layoutEditorContainer').find('.contents'),
            relatedBlock = contents.find('.block_' + result['blockid']),
            fieldCopy = contents.find('.newCustomFieldCopy').clone(true, true),
            fieldContainer = fieldCopy.find('.fieldLabelContainer');
        ...
        let block = relatedBlock.find('.blockFieldsList'),
            sortable1 = block.find('ul[name=sortable1]'),
            sortable2 = block.find('ul[name=sortable2]');

        if (sortable1.length < 1 && sortable2.length < 1) {
            sortable1 = block.find('ul[name=unSortable1]');
            sortable2 = block.find('ul[name=unSortable2]');
            fieldCopy.find('.dragImage').hide();
        }

        let firstSortableChildren = sortable1.children(),
        ...
        let length1 = firstSortableChildren.filter(':not(l1.dummyRow)').length,
            length2 = secondSortableChildren.filter(':not(l1.dummyRow)').length;
        ...
        let form = fieldCopy.find('.fieldProperties');
    ...
    setFieldDetails: function (result, form) {
        ...
        if (!result['fieldDefaultValue']) {
            ...
            var html = '<div class="defaultValueContent">';
            html += '<span class="me-2">' + defaultValueIcon + '</span>';
            ...
            if (isObject && typeof defaultValue == 'object') {
                ...
                jQuery.each(defaultValue, function (fieldName, value) {
                    ...
                    html += '<div class="defaultValueContent">';
                    html += '<span class="me-2">' + defaultValueIcon + '</span>';
                    if (value) {
                        html += '<span class="me-2" data-defaultvalue-fieldname="' + fieldName + '" data-defaultvalue="' + value + '">' + app.vtranslate('JS_DEFAULT_VALUE') + app.vtranslate("JS_" + fieldName) + ":</span><span>" + value + '</span>';
                    } else {
                        html += '<span>' + app.vtranslate('JS_DEFAULT_VALUE_NOT_SET') + '</span>';
                    }
                    html += '</div>';
    ...
    registerModulesChangeEvent: function () {
        let thisInstance = this,
            container = jQuery('#layoutEditorContainer'),
            contentsDiv = container.closest('.settingsPageDiv');
    ...
    registerEditFieldDetailsClick: function (contents) {
        ...
        contents.on('click', '.editFieldDetails', function (e) {
            let element = jQuery(e.currentTarget),
                ...
                fieldId = container.find('.editFields').data('fieldId'),
                blockId = element.closest('.editFieldsTable').data('blockId');
