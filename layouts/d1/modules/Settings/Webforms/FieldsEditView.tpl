...
{strip}
    ...
    <div class="fieldBlockContainer-webform">
        <div class="fieldBlockHeader py-3 px-4">
            <h4 class="m-0">{vtranslate($SOURCE_MODULE, $SOURCE_MODULE)} {vtranslate('LBL_FIELD_INFORMATION', $MODULE)}</h4>
        </div>
        <div class="px-3">
            <table class="table table-bordered" name="targetModuleFields">
                <colgroup>
                    <col style="width:5%;">
                    <col style="width:5%;">
                    <col style="width:25%;">
                    ...
                    <td colspan="5">
                        <div class="row">
                            <div class="col-sm-2 col-lg-2">
                                <div class="textAlignCenter"><b>{vtranslate('LBL_ADD_FIELDS', $MODULE)}</b></div>
                            </div>
                            <div class="col-sm-8 col-lg-8 fieldsListContainer select2-mandatory-allow-clear-false">
                                <select id="fieldsList" multiple="multiple" data-placeholder="{vtranslate('LBL_SELECT_FIELDS_OF_TARGET_MODULE', $MODULE)}" class="select2" style="width:100%" data-dropdown-css-class="select2-hide-selected" data-dropdown-parent=".fieldsListContainer">
                                    {foreach key=BLOCK_LABEL item=BLOCK_FIELDS from=$ALL_FIELD_MODELS_LIST name="EditViewBlockLevelLoop"}
                                        {foreach key=FIELD_NAME item=FIELD_MODEL from=$BLOCK_FIELDS name=blockfields}
                                            ...
                                            <option value="{$FIELD_MODEL->get('name')}" data-field-info='{$FIELD_INFO}' data-mandatory="{($FIELD_MODEL->isMandatory(true) eq 1) ? "true":"false"}" {if (array_key_exists($FIELD_MODEL->get('name'), $SELECTED_FIELD_MODELS_LIST)) or ($FIELD_MODEL->isMandatory(true))}selected{/if}>
                                ...
                                <input type="hidden" id="fieldsListSelected" value={json_encode(array_keys((array)$SELECTED_FIELD_MODELS_LIST))}>
                            </div>
                            <div class="col-sm-2 col-lg-2">
                        ...
                        {if $FIELD_MODEL->isMandatory(true) || array_key_exists($FIELD_NAME,$SELECTED_FIELD_MODELS_LIST)}
                            ...
                            <tr data-name="{$FIELD_MODEL->getFieldName()}" class="listViewEntries" data-type="{$FIELD_MODEL->getFieldDataType()}" data-mandatory-field={($FIELD_MODEL->isMandatory(true) eq 1) ? "true":"false"}>
                                ...
                                <td class="fieldValue" data-name="{$FIELD_MODEL->getFieldName()}" {if in_array($FIELD_MODEL->getFieldDataType(),$DATATYPEMARGINLEFT)} {/if}>
                                    ...
                                    {elseif $FIELD_MODEL->getFieldDataType() != 'image'}
    ...
    {if Vtiger_Functions::isDocumentsRelated($SOURCE_MODULE)}
        <div class="fieldBlockContainer">
            <div class="fieldBlockHeader">
                <h4 class="px-4 py-3 m-0">{vtranslate('LBL_UPLOAD_DOCUMENTS', $QUALIFIED_MODULE)}</h4>
            </div>
            <div class="p-3">
                <button class="btn btn-outline-secondary" id="addFileFieldBtn">
                    ...
                    <table class="table table-bordered" id='fileFieldsTable'>
                        <tbody>
                            ...
                            {foreach from=$DOCUMENT_FILE_FIELDS key=FILE_INDEX item=DOCUMENT_FILE_FIELD}
                                <tr>
                                    <td style="vertical-align: middle;">
                                        <input type="text" class="inputElement form-control nameField" name="file_field[{$FILE_INDEX}][fieldlabel]" value="{$DOCUMENT_FILE_FIELD['fieldlabel']}" data-rule-required="true">
                                    </td>
                            ...
                            <tr class="noFileField {if php7_count($DOCUMENT_FILE_FIELDS) gt 0}hide{/if}">
                                <td class="text-center" colspan="3" style="height: 100px; vertical-align: middle;">
                                    {vtranslate('LBL_NO_FILE_FIELD', $QUALIFIED_MODULE)}
                                </td>
                            </tr>
                        </tbody>
                    </table>
