...
<div class="listViewTagsTemplate">
    <div id="listViewTagContainer" class="multiLevelTagList" {if $ALL_CUSTOMVIEW_MODEL} data-view-id="{$ALL_CUSTOMVIEW_MODEL->getId()}" {/if} data-list-tag-count="{Vtiger_Tag_Model::NUM_OF_TAGS_LIST}">
        {assign var=ALL_CUSTOM_VIEW_ID value=CustomView_Record_Model::getAllFilterByModule($MODULE)->get('cvid')}
        <div class="tag btn mt-2 me-1 {if $VIEWNAME eq $ALL_CUSTOM_VIEW_ID}btn-primary{else}text-primary bg-primary bg-opacity-10{/if}" data-cv-id="{$ALL_CUSTOM_VIEW_ID}" data-id="">
        ...
        {if $ALL_CUSTOM_VIEW_ID neq $DEFAULT_CUSTOM_VIEW_ID}
            <div class="tag btn mt-2 me-1 {if $VIEWNAME eq $DEFAULT_CUSTOM_VIEW_ID}btn-primary{else}text-primary bg-primary bg-opacity-10{/if}" data-cv-id="{$DEFAULT_CUSTOM_VIEW_ID}" data-id="">
                <i class="fa fa-list"></i>
        ...
        {if $VIEWNAME and $VIEWNAME neq $ALL_CUSTOM_VIEW_ID and $VIEWNAME neq $DEFAULT_CUSTOM_VIEW_ID}
            ...
            <div class="tag btn mt-2 me-1 {if $VIEWNAME eq $CURRENT_CUSTOM_VIEW_ID}btn-primary{else}text-primary bg-primary bg-opacity-10{/if}" data-cv-id="{$CURRENT_CUSTOM_VIEW_ID}" data-id="">
                <i class="fa fa-list"></i>
