...
{if $FIELD_MODEL->get('uitype') eq '55'}
	...
	<select class="select2 inputElement" data-name="{$ASSIGNED_USER_ID}" name="{$ASSIGNED_USER_ID}[]" multiple="multiple"
		...
		<optgroup label="{vtranslate('LBL_USERS')}">
			{foreach key=OWNER_ID item=OWNER_NAME from=$ALL_ACTIVEUSER_LIST}
				<option value="{$OWNER_ID}" data-picklistvalue='{$OWNER_NAME}' {foreach item=USER from=$FIELD_VALUE}{if $USER eq $OWNER_ID } selected="selected" {/if}{/foreach}
