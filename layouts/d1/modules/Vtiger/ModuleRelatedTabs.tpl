...
*************************************************************************************}
{strip}
    <div class='related-tabs col-lg-3 col-xl-2 order-2'>
        <div class="bg-white rounded h-100 px-3">
            <ul class="nav nav-pills flex-column py-3">
                {foreach item=RELATED_LINK from=$DETAILVIEW_LINKS['DETAILVIEWTAB']}
                    ...
                    <li class="tab-item nav-link fs-6 {if $RELATEDLINK_LABEL==$SELECTED_TAB_LABEL}active text-primary{else}text-secondary{/if}" data-url="{$RELATEDLINK_URL}&tab_label={$RELATED_TAB_LABEL}&app={$SELECTED_MENU_CATEGORY}" data-label-key="{$RELATEDLINK_LABEL}" data-link-key="{$RELATED_LINK->get('linkKey')}">
                ...
                {if !empty($RELATED_LINKS)}
                    {foreach from=$RELATED_LINKS item=RELATED_LINK}
                        ...
                        <li class="tab-item nav-link fs-6 {if (trim($RELATED_LINK->getLabel())== trim($SELECTED_TAB_LABEL)) && ($RELATED_LINK->getId() == $SELECTED_RELATION_ID)}active text-primary{else}text-secondary{/if}" data-url="{$RELATED_LINK->getUrl()}&tab_label={$RELATED_LINK->getLabel()}&app={$SELECTED_MENU_CATEGORY}" data-label-key="{$RELATED_LINK->getLabel()}"
