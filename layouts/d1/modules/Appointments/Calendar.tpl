                ...
                <form method="post" id="CalendarFilter">
                    ...
                    <div class="container-fluid calendarTypeContainer">
                        {foreach from=$FIELD_TYPE_VALUES key=TYPE_KEY item=TYPE_VALUE}
                            <label class="row align-items-center py-2 rounded" style="{if !empty($TYPE_COLORS[$TYPE_KEY])}--bs-primary-color:{$TYPE_COLORS[$TYPE_KEY]};{/if}">
                                <div class="col-auto" style="{if !empty($TYPE_COLORS[$TYPE_KEY])}--bs-border-color:{$TYPE_COLORS[$TYPE_KEY]};{/if}">
                                    <input type="checkbox" checked="checked" value="{$TYPE_KEY}" name="field_calendar_type" class="form-check-input fieldCalendarType fieldCalendarsType">
                                </div>
                                <div class="col">
                                    <div class="text-truncate">{$TYPE_VALUE}</div>
                                </div>
                            </label>
                        {/foreach}
                    </div>
                    {if $IS_EVENT_TYPES_VISIBLE}
                        <div class="container-fluid eventTypeContainer">
                            <div class="row align-items-center py-2 rounded eventType eventTypeClone">
                                <div class="col-auto">
                                    <input type="checkbox" class="form-check-input fieldEventType eventTypeId" checked="checked">
                                </div>
                            ...
                            {foreach from=$EVENT_TYPES item=EVENT_TYPE}
                                <div class="row align-items-center py-2 rounded eventType" style="--bs-primary-color:{$EVENT_TYPE->getBackgroundColor()};">
                                    <div class="col-auto" style="--bs-border-color:{$EVENT_TYPE->getBackgroundColor()};">
                    ...
                    {/if}
                    <div id="users_groups_tabs" class="mt-3">
                        ...
                        <div class="mb-2 select_users_actions hide">
                            <div class="select_users_and_groups btn btn-outline-secondary w-100 d-flex justify-content-between">
                                <span class="ms-2">{vtranslate('LBL_FILTER', $QUALIFIED_MODULE)}</span>
