...
Vtiger_Index_Js('Appointments_Calendar_Js', {
    deleteEvent: function (record, module) {
        ...
        app.helper.showConfirmationBox({message: app.vtranslate('JS_DELETE_QUESTION')}).then(function () {
            ...
            app.request.post({data: params}).then(function (error, data) {
                if (!error && data) {
                    app.event.trigger('Appointments.Event.Delete', {id: record})
    ...
    getInstance: function () {
        if (!this.instance) {
            this.instance = new Appointments_Calendar_Js();
        }
    ...
    registerDeleteEvent: function () {
        ...
        app.event.on('Appointments.Event.Delete', function (event, data) {
    ...
    showQuickEdit: function (info) {
        const startDateString = info.startStr,
            ...
            quickCreateNode = jQuery('#quickCreateModules').find('[data-name="Appointments"]'),
            quickCreateParams = {
                'noCache': false,
            };
