                                        ...
                                        <span class="value text-truncate">
                                            ...
                                            {elseif $LISTVIEW_HEADER->getFieldDataType() eq 'multipicklist'}
                                                ...
                                                {foreach item=MULTI_PICKLIST_VALUE key=MULTI_PICKLIST_INDEX from=$MULTI_PICKLIST_VALUES}
                                                    <span class="me-1 py-1 px-2 rounded picklist-color picklist-{$LISTVIEW_HEADER->getId()}-{Vtiger_Util_Helper::convertSpaceToHyphen(trim($MULTI_PICKLIST_VALUE))}">{trim($MULTI_PICKLIST_VALUES[$MULTI_PICKLIST_INDEX])}</span>
                                                {/foreach}
                                            {else}
