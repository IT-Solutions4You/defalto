                    ...
                    {foreach from=$FORMATTED_RECORD_STRUCTURE item=INVENTORY_ROW}
                        <div class="d-flex flex-row py-2">
                            ...
                            {if $INVENTORY_ROW.1.0 neq ''}
                                ...
                                <div class="col-lg-6">
                                    {if $FIELD_NAME eq 'discount'}
                                        <div class="input-group">
                                            <input id="{$FIELD_NAME}" name="{$FIELD_NAME}" type="text" class="{$FIELD_NAME} inputElement form-control textAlignRight"
                                                   value="{$data[$FIELD_NAME]}" readonly="readonly"/>
                                        ...
                                        <div class="position-relative">
                                            <div class="popover lineItemPopover border-1 bs-popover-auto fade discountSettingsDiv" id="discountSettingsDiv" role="tooltip"
                                                 style="position: absolute; inset: 0px 0px auto auto; margin: 0px; opacity: 1; visibility: visible; transform: translate(-51px, -126px); display: none;"
                                                 data-popper-placement="left">
                                                <h3 class="popover-header p-3 m-0 border-bottom">{vtranslate('Discount of', 'InventoryItem')} <span
                                                        class="subtotal_in_discount_div">{$data.subtotal}</span></h3>
                                                        ...
                                                        <table class="table table-borderless popupTable m-0">
                                                            ...
                                                            <tr>
                                                                ...
                                                                <td>
                                                                    ...
                                                                    <input type="hidden" name="original_discount_type" id="original_discount_type" value="{$data.discount_type}"
                                                                           class="original_discount_type">
                                                                </td>
                                                            </tr>
                                                            <tr>
                                                                <td class="p-3">{vtranslate('Discount', 'InventoryItem')}</td>
                                                                <td>
                                                                    <input type="text" size="5" data-compound-on="" name="discount_popup" id="discount_popup"
                                                                           value="{$data.discount}"
                                                                           class="form-control discount_popup replaceCommaWithDot textAlignRight doNotRecalculateOnChange"
                                                                           data-rule-positive="true" data-rule-inventory_percentage="true" aria-invalid="false">
                                                                    <input type="hidden" id="original_discount" name="original_discount" value="{$data.discount}"
                                                                           class="original_discount">
                                                                </td>
                                                            </tr>
                                                            <tr>
                                                                <td class="p-3">{vtranslate('Computed value', 'InventoryItem')}</td>
                                                                <td class="text-end">
                                                                    <input type="text" size="6" name="discount_computed_value" id="discount_computed_value" style="cursor:pointer;"
                                                                           value="{$data.discount_amount}" readonly="readonly"
                                                                           class="form-control discount_computed_value textAlignRight" aria-invalid="false">
                                                                </td>
                                                            </tr>
                                                            </tbody>
                                                        </table>
                                                    </div>
                                                    <input id="original_{$FIELD_NAME}" name="original_{$FIELD_NAME}" type="hidden" class="original_{$FIELD_NAME}"
                                                           value="{$data[$FIELD_NAME]}"/>
                                                </div>
                            ...
                            {if $INVENTORY_ROW.1.1 neq ''}
                                ...
                                <div class="col-lg-2">
                                    <input id="{$FIELD->get('name')}" name="{$FIELD->get('name')}" type="text" class="{$FIELD->get('name')} inputElement form-control"
