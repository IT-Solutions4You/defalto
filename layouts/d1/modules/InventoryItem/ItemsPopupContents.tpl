                        ...
                        <tr class="listViewEntries border-top" data-id="{$LISTVIEW_ENTRY->getId()}" data-name="{$LISTVIEW_ENTRY->getName()}" data-info='{Vtiger_Util_Helper::toSafeHTML(ZEND_JSON::encode($LISTVIEW_ENTRY->getRawData()))}'
                            ...
                            {foreach item=LISTVIEW_HEADER from=$LISTVIEW_HEADERS name=listViewEntry}
                                ...
                                <td class="listViewEntryValue text-truncate">
                                    ...
                                    {elseif $LISTVIEW_HEADER->getFieldDataType() eq 'picklist' and $FIELD_MODEL->isPicklistColorSupported()}
                                    ...
                                    {elseif $LISTVIEW_HEADER->getFieldDataType() eq 'multipicklist'}
                                        ...
                                        {foreach item=MULTI_PICKLIST_VALUE key=MULTI_PICKLIST_INDEX from=$MULTI_RAW_PICKLIST_VALUES}
                                            <span class="py-1 px-2 rounded picklist-color picklist-{$LISTVIEW_HEADER->getId()}-{Vtiger_Util_Helper::convertSpaceToHyphen(trim($MULTI_PICKLIST_VALUE))}">{trim($MULTI_PICKLIST_VALUES[$MULTI_PICKLIST_INDEX])}</span>
                                        {/foreach}
