...
class HelpDeskHandler extends VTEventHandler
{
    ...
    public function createEmailFromMailManager(int $recordId): void
    {
        ...
        if (empty(
            MailManager_Relation_View::getMailSession()
            ) || empty($requestData['sourceModule']) || empty($requestData['sourceRecord']) || 'MailManager' !== $requestData['sourceModule']) {
            return;
        }
...
function HelpDesk_notifyParentOnTicketChange($entityData)
{
    ...
    if (!empty($contactId[0])) {
        $result = $adb->pquery('SELECT email, emailoptout, lastname, firstname FROM vtiger_contactdetails WHERE contactid=?', [$contactId[1]]);
