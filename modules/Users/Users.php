...
{
    public string $moduleVersion = '1.3';
    ...
    function change_password($user_password, $new_password, $dieOnError = true)
    {
        ...
        if (!is_admin($current_user)) {
            ...
            if ($this->db->hasFailedTransaction()) {
                if ($dieOnError) {
                    die(
                        "error verifying old transaction[" . $this->db->database->ErrorNo() . "] " .
        ...
        if ($this->db->hasFailedTransaction()) {
            if ($dieOnError) {
                die(
                    "error setting new password: [" . $this->db->database->ErrorNo() . "] " .
                    $this->db->database->ErrorMsg()
                );
            }
    ...
    public function setUserPreferences($requestArray)
    {
        ...
        if (isset($requestArray['about']['country_id'])) {
            $updateData['address_country_id'] = vtlib_purify($requestArray['about']['country_id']);
        }
