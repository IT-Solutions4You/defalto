...
- Report preview and execution use `Core_QueryGenerator_Model::parseAdvFilterList()`, which owns terminal connector cleanup and outer grouping. Saved filters can retain an outgoing connector on the final group; normalize at this shared query boundary before SQL generation.
...
- `getFormatedFilters()` preserves stored groups and connectors. Empty intermediate groups and sparse keys are handled by the shared query parser, not a two-group assumption.
