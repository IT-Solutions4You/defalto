...
class Vtiger_Install_Model extends Core_Install_Model
{
    ...
    {
        foreach (Core_CreateTables_Config::getAll() as $className) {
    ...
    public function migrate()
    {
        ...
        foreach (Core_DeleteFiles_Config::getAll() as $deleteFile) {
        ...
        self::logSuccess('Create links start');

        foreach (Core_CreateLinks_Config::getAll() as $className) {
            if (class_exists($className) && method_exists($className, 'createLinks')) {
                (new $className())->createLinks();
            }
        }

        self::logSuccess('Create links end');
    }
}