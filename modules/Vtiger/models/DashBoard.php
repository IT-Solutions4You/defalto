...
class Vtiger_DashBoard_Model extends Vtiger_Base_Model
{
    ...
    public function getSelectableDashboard()
    {
        ...
        $sql = 'SELECT * FROM vtiger_links WHERE linktype = ? AND tabid IN (' . generateQuestionMarks(
                $moduleTabIdList
            ) . ') AND linkid NOT IN (SELECT linkid FROM vtiger_module_dashboard_widgets WHERE userid = ? and dashboardtabid=? )';
