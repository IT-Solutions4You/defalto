...
class Vtiger_Util_Helper
{
    ...
    public static function getCreator($recordId)
    {
        ...
        $db = PearDatabase::getInstance();
        $result = $db->pquery('SELECT creator_user_id FROM vtiger_crmentity WHERE crmid = ?', [$recordId]);
        ...
        if ($db->num_rows($userEntries) <= 0) {
            ...
            foreach ($activityIds as $activityId => $color) {
                $db->pquery(
                    ...
                    [$db->getUniqueID('vtiger_calendar_user_activitytypes'), $activityId, $userId, $color]
                );
            }
        }
    }
    ...
    */
    ...
    static $detectFieldnamesToResolveCache = [];
