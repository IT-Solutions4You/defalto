...
In the saved-list condition editor, each click on the add-group button adds one group with an empty condition. Use AND or OR inside each group and between groups to combine your criteria. The connector between groups sits directly below the preceding group with compact spacing. The group operator and trash button use matching compact heights.
