...
class WSAPP_Logs
{
    ...
    static function add($syncRecords)
    {
        ...
        $params1 = [
            ...
            $extensionModule,
            $syncTime,
            $recordsCount['app']['create'],
            $recordsCount['app']['update'],
            $recordsCount['app']['delete'],
            $recordsCount['app']['skipped'],
            $recordsCount['vtiger']['create'],
            $recordsCount['vtiger']['update'],
            $recordsCount['vtiger']['delete'],
            $recordsCount['vtiger']['skipped'],
            $userId
        ];
    ...
    function getSyncRecordsDetails($syncRecords)
    {
        $a = $b = $c = $d = $i = $j = $k = $l = 0;
        $recordCount = [
            'vtiger' => ['update' => 0, 'create' => 0, 'delete' => 0, 'skipped' => 0],
            'app'    => ['update' => 0, 'create' => 0, 'delete' => 0, 'skipped' => 0]
        ];
        $recordDetails = [
            'vt_record'  => ['update' => [], 'create' => [], 'delete' => [], 'skipped' => []],
                ...
                foreach ($records as $record) {
                    foreach ($record as $type => $data) {
                        ...
                        if ($type == 'source') {
                            ...
                            switch ($data->getMode()) {
                                case WSAPP_SyncRecordModel::WSAPP_UPDATE_MODE    :
                                    $recordCount['vtiger']['update']++;
                                    $recordDetails['vt_record']['update'][$i++] = $recordInfo['id'];
                                    break;
                                case WSAPP_SyncRecordModel::WSAPP_CREATE_MODE    :
                                    $recordCount['vtiger']['create']++;
                                    $recordDetails['vt_record']['create'][$j++] = $recordInfo['id'];
                                    break;
                                case WSAPP_SyncRecordModel::WSAPP_DELETE_MODE    :
                                    $recordCount['vtiger']['delete']++;
                                    $recordDetails['vt_record']['delete'][$k++] = $source['_serverid'];
                                    break;
                                case 'skipped'                                    :
                                    if (empty($recordInfo['messageidentifier'])) {
