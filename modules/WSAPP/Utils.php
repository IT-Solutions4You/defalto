...
function wsapp_getRecordEntityNameIds($entityNames, $modules, $user)
{
    ...
    foreach ($modules as $moduleName) {
        ...
        $query = "SELECT " . $meta->getObectIndexColumn() . " as id,$nameFields as entityname FROM " . $meta->getEntityBaseTable(
            ) . " as moduleentity INNER JOIN vtiger_crmentity as crmentity WHERE $nameFields IN(" . generateQuestionMarks(
                $entityNames
            ) . ") AND crmentity.deleted=0 AND crmentity.crmid = moduleentity." . $meta->getObectIndexColumn() . "";
...
function wsapp_checkIfRecordsAssignToUser($recordsIds, $userIds)
{
    ...
    $db = PearDatabase::getInstance();
    $query = "SELECT * FROM vtiger_crmentity where crmid IN (" . generateQuestionMarks($recordsIds) . ") and assigned_user_id in (" . generateQuestionMarks($userIds) . ")";
    $params = [];
