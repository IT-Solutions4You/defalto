...
class SyncServer
{
	...
	function put($key, $element, $user)
	{
		...
		foreach ($records as $record) {
			...
			$lookupResult = $db->pquery(
				"SELECT serverid,clientmodifiedtime FROM vtiger_wsapp_recordmapping WHERE appid=? AND clientid COLLATE utf8_bin =?",
				[$appid, $clientRecordId]
			);
	...
	function getQueueDeleteRecord($appId)
	{
		$db = PearDatabase::getInstance();
		$result = $db->pquery(
			"SELECT * FROM vtiger_wsapp_queuerecords
