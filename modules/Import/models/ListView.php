...
class Import_ListView_Model extends Vtiger_ListView_Model
{
    ...
    public function getLastImportedRecord()
    {
        ...
        $result = $db->pquery(
            'SELECT recordid FROM ' . $userDBTableName . ' WHERE status NOT IN (?,?) AND recordid IS NOT NULL',
            [Import_Data_Action::$IMPORT_RECORD_FAILED, Import_Data_Action::$IMPORT_RECORD_SKIPPED]
        );
