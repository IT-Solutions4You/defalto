    ...
    function process(CustomerPortal_API_Request $request)
    {
        ...
        if ($current_user) {
            ...
            if ($module == 'ModComments' && !empty($recordId)) {
                ...
                $result = vtws_query(
                    sprintf(
                        "SELECT * FROM ModComments WHERE related_to = '%s' AND is_private='%s' ORDER BY %s DESC LIMIT %s,%s;",
                        $recordId,
                        0,
                        'modifiedtime',
                        ($page * $pageLimit),
                ...
                if (is_array($result)) {
                    foreach ($result as $index => $value) {
                        ...
                        if (!empty($attachmentIds)) {
                            $attachmentsResult = $adb->pquery(
                                'SELECT attachmentsid,name FROM vtiger_attachments WHERE attachmentsid IN (' . generateQuestionMarks($attachmentIds) . ')',
                                $attachmentIds
                            );
