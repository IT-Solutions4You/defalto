...
class ModTracker extends CRMExtension
{
    ...
    static function isTrackingEnabledForModule($modulename)
    {
        ...
        if (!self::getVisibilityForModule($tabid) || self::getVisibilityForModule($tabid) !== 0) {
            $query = $adb->pquery(
                "SELECT * FROM vtiger_modtracker_tabs WHERE vtiger_modtracker_tabs.visible = 1
    ...
    static function isModtrackerLinkPresent($tabid)
    {
        global $adb;
        $query1 = $adb->pquery(
            "SELECT * FROM vtiger_links WHERE linktype='DETAILVIEWBASIC' AND
							  linklabel = 'View History' AND tabid = ?",
            [$tabid]
        );
    ...
    static function trackRelation($sourceModule, $sourceId, $targetModule, $targetId, $type)
    {
        ...
        $adb->pquery(
            'INSERT INTO vtiger_modtracker_relations(id, targetmodule, targetid, changedon)
            VALUES(?,?,?,?)',
            [$id, $targetModule, $targetId, $currentTime]
        );
    }
