...
for ($i = 0; $i < $count; $i++) {
    ...
    if (!empty($tableAndColumnSize[$tableName][$columnName]['scale'])) {
        ...
        if ($decimalsToChange != 0) {
            ...
            Migration_Index_View::ExecuteQuery(
                'ALTER TABLE ' . $tableName . ' CHANGE ' . $columnName . ' ' . $columnName . ' decimal(' . (int)$maxlength . ',' . (int)$decimalDigits . ')',
                null
            );
        }
    }
...
Migration_Index_View::ExecuteQuery(
    ...
    [$webserviceObject->getEntityId(), 'vtiger_inventoryproductrel']
);
...
Migration_Index_View::ExecuteQuery(
    ...
    [$fieldTypeId, 'vtiger_inventoryproductrel', 'productid', "reference"]
);
...
Migration_Index_View::ExecuteQuery(
    "INSERT INTO vtiger_ws_entity_fieldtype(fieldtypeid,table_name, field_name,fieldtype) VALUES (?,?,?,?);",
    [$fieldTypeId, 'vtiger_inventoryproductrel', 'id', "reference"]
);
...
Migration_Index_View::ExecuteQuery(
    "INSERT INTO vtiger_ws_entity_fieldtype(fieldtypeid,table_name,field_name,fieldtype) VALUES (?,?,?,?);",
    [$fieldTypeId, 'vtiger_inventoryproductrel', 'incrementondel', "autogenerated"]
);
...
Migration_Index_View::ExecuteQuery(
    "INSERT INTO vtiger_ws_entity_tables(webservice_entity_id,table_name) VALUES (?,?)",
    [$webserviceObject->getEntityId(), 'vtiger_inventorytaxinfo']
);
...
Migration_Index_View::ExecuteQuery(
    "INSERT INTO vtiger_ws_entity_fieldtype(fieldtypeid,table_name,field_name,fieldtype) VALUES (?,?,?,?);",
    [$fieldTypeId, 'vtiger_producttaxrel', 'productid', "reference"]
);
Migration_Index_View::ExecuteQuery("INSERT INTO vtiger_ws_entity_referencetype(fieldtypeid,type) VALUES (?,?)", [$fieldTypeId, 'Products']);
...
Migration_Index_View::ExecuteQuery(
    "INSERT INTO vtiger_ws_entity_fieldtype(fieldtypeid,table_name,field_name,fieldtype) VALUES (?,?,?,?);",
    [$fieldTypeId, 'vtiger_producttaxrel', 'taxid', "reference"]
);
...
Vtiger_Utils::CreateTable(
    ...
    '(id INT(19) NOT NULL AUTO_INCREMENT, linkid INT(19), userid INT(19), filterid INT(19),
...
Migration_Index_View::ExecuteQuery(
    "DELETE FROM vtiger_cvcolumnlist WHERE cvid IN
...
do {
    ...
    for ($i = 0; $i < $rows; $i++) {
        ...
        Migration_Index_View::ExecuteQuery(
            'UPDATE vtiger_crmentity SET modifiedtime = ?, creator_user_id = ?, modifiedby = ? WHERE crmid = ?',
...
do {
    $rs = $adb->pquery(
        "SELECT crmid,setype FROM vtiger_crmentity INNER JOIN vtiger_tab ON vtiger_tab.name=vtiger_crmentity.setype WHERE label IS NULL AND crmid > ? LIMIT 500",
        [$lastMaxCRMId]
    );
    if (!$adb->num_rows($rs)) {
        break;
...
Migration_Index_View::ExecuteQuery('UPDATE vtiger_users SET dayoftheweek = ?, callduration = ?, othereventduration = ?, start_hour = ? ', ['Monday', 5, 5, '00:00']);
...
$result = $adb->pquery(
    'SELECT vtiger_homestuff.stufftitle, vtiger_homestuff.userid, vtiger_notebook_contents.contents FROM
						vtiger_homestuff INNER JOIN vtiger_notebook_contents on vtiger_notebook_contents.notebookid = vtiger_homestuff.stuffid
						WHERE vtiger_homestuff.stufftype = ?',
    ['Notebook']
);
...
Migration_Index_View::ExecuteQuery("UPDATE vtiger_leaddetails SET salutationtype='' WHERE salutationtype = ?", ['--None--']);

//Update contacts salutation value of none to empty value
Migration_Index_View::ExecuteQuery("UPDATE vtiger_contactdetails SET salutationtype='' WHERE salutationtype = ?", ['--None--']);
// END 2013-06-25
...
$lastTicketId = 0;
do {
    $ticketsResult = $adb->pquery(
        "SELECT ticketid ,parent_id FROM vtiger_troubletickets WHERE ticketid > ?
						LIMIT 500",
        [$lastTicketId]
    );
...
$result = $adb->pquery(
    'SELECT DISTINCT workflow_id FROM com_vtiger_workflowtasks WHERE workflow_id IN
				(SELECT workflow_id FROM com_vtiger_workflows WHERE module_name IN (?) AND defaultworkflow = ?)
				AND task LIKE ?',
    [$moduleName, 1, '%VTEntityMethodTask%']
);
$numOfRows = $adb->num_rows($result);
