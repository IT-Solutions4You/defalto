...
if (defined('VTIGER_UPGRADE')) {
    ...
    $db->pquery(
        'UPDATE vtiger_ws_entity SET handler_path=?, handler_class=? WHERE name IN("Products","Services")',
    ...
    foreach ($updateList as $list) {
        $db->pquery(
            'UPDATE vtiger_calendar_default_activitytypes SET fieldname=? WHERE module=? AND fieldname=? AND isdefault=?',
            [Zend_Json::encode($list['newfieldname']), $list['module'], $list['fieldname'], '1']
        );
    }
    ...
    foreach ($refModulesList as $refModuleName) {
        ...
        if (!$db->num_rows($result)) {
            $db->pquery(
                ...
                [$db->getUniqueID('vtiger_relatedlists'), $refModuleTabId, $modCommentsTabId, 'get_comments', '1', 'ModComments', '0', '', $fieldId, 'NULL', '1:N']
            );
        }
    }
    ...
    $lastMaxCRMId = 0;
    do {
        $commentsResult = $db->pquery(
            'SELECT vtiger_modcomments.modcommentsid FROM vtiger_modcomments 
												...
												OR vtiger_crmentity.setype IS NULL) AND modcommentsid > ? LIMIT 500',
            array_merge($internalCommentModules, [$lastMaxCRMId])
        );
    ...
    if ($row['cvid']) {
        $columnNameCount = $db->pquery(
            'SELECT 1 FROM vtiger_cvcolumnlist WHERE cvid=? and columnname=?',
            [$row['cvid'], 'vtiger_project:contactid:contactid:Project_Contact_Name:V']
        );
        if (!$db->num_rows($columnNameCount)) {
            ...
            $db->pquery(
                ...
                [$row['cvid'], $colIndex['columnindex'] + 11, 'vtiger_project:contactid:contactid:Project_Contact_Name:V']
            );
        }
    }
    //End
    ...
    if (!Vtiger_Utils::CheckTable('vtiger_customerportal_relatedmoduleinfo')) {
        ...
        $availableFourOperations = [
            ['name' => 'History', 'value' => 1],
            ['name' => 'ModComments', 'value' => 1],
            ['name' => 'ProjectTask', 'value' => 1],
            ['name' => 'ProjectMilestone', 'value' => 1]
        ];
        $availableFiveOperations = [
            ['name' => 'History', 'value' => 1],
            ['name' => 'ModComments', 'value' => 1],
            ['name' => 'ProjectTask', 'value' => 1],
            ['name' => 'ProjectMilestone', 'value' => 1],
    ...
    if (!Vtiger_Utils::CheckTable('vtiger_customerportal_settings')) {
        $db->pquery(
            'CREATE TABLE vtiger_customerportal_settings(id int, url VARCHAR(250),default_assignee INT(11),
        ...
        $db->pquery(
            'INSERT INTO vtiger_customerportal_settings(id,default_assignee,shortcuts,widgets,charts) VALUES(?,?,?,?,?)',
            [1, 1, $encodedShortcuts, $encodedWidgets, $encodedCharts]
        );
    }
    ...
    $deletedPicklistResult = $db->pquery(
        'SELECT DISTINCT(picklistid) AS picklistid FROM vtiger_role2picklist 
    ...
    if (!Vtiger_Utils::checkTable('vtiger_inventorycharges')) {
        ...
        $db->pquery(
            ...
            [1, 'Shipping & Handling', 'Flat', 'Fixed', null, '[]', 1, ZEND_JSON::encode($taxIdsList), 0]
        );
    }
    ...
    $userManagementFields = [
        'LBL_USERS'                 => 'index.php?module=Users&parent=Settings&view=List',
    ...
    $moduleManagerFields = [
        'VTLIB_LBL_MODULE_MANAGER'    => 'index.php?module=ModuleManager&parent=Settings&view=List',
    ...
    $automationFields = [
        'Webforms'           => 'index.php?module=Webforms&parent=Settings&view=List',
    ...
    $configurationFields = [
        'LBL_COMPANY_DETAILS'      => 'index.php?parent=Settings&module=Vtiger&view=CompanyDetails',
    ...
    $marketingSalesFields = [
        'LBL_LEAD_MAPPING'        => 'index.php?parent=Settings&module=Leads&view=MappingDetail',
    ...
    $myPreferenceFields = [
        'My Preferences'    => 'index.php?module=Users&view=PreferenceDetail&parent=Settings&record=1',
    ...
    $blocksAndNameFields = [
        $userManagementBlockId => array_keys($userManagementFields),
    ...
    $db->pquery(
        'DELETE FROM vtiger_settings_field WHERE name IN (' . generateQuestionMarks($duplicateOtherBlockFields) . ') AND blockid=?',
        [$duplicateOtherBlockFields, $otherBlockId]
    );
    ...
    foreach ($allEntityModules as $tabId => $moduleModel) {
        ...
        if ($baseTableName) {
            ...
            if ($db->num_rows($checkIfConstraintExists) < 1) {
                $db->pquery(
                    "ALTER TABLE $baseTableName ADD CONSTRAINT fk_crmid_$baseTableName FOREIGN KEY ($baseTableIndex) REFERENCES vtiger_crmentity (crmid) ON DELETE CASCADE",
                    []
                );
            }
            ...
            $db->pquery(
                "DELETE FROM $baseTableName WHERE $baseTableIndex NOT IN (SELECT crmid FROM vtiger_crmentity WHERE setype in (" . generateQuestionMarks($deleteQueryParams) . "))",
                $deleteQueryParams
            );
        }
    }
