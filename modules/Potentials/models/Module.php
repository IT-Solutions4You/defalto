...
class Potentials_Module_Model extends Vtiger_Module_Model
{
    ...
    public function getPotentialsCountBySalesStage($owner, $dateFilter)
    {
        ...
        $result = $db->pquery(
            'SELECT COUNT(*) count, vtiger_potential.sales_stage FROM vtiger_potential
                ...
                $this->getName()
            ) . $ownerSql . $dateFilterSql . ' AND vtiger_potential.sales_stage IN (' . generateQuestionMarks($picklistvaluesmap) . ') 
					    GROUP BY sales_stage ORDER BY vtiger_sales_stage.sortorderid',
            $params
        );
    ...
    public function getPotentialsCountBySalesPerson()
    {
        ...
        $result = $db->pquery(
            'SELECT COUNT(*) AS count, vtiger_users.userlabel as last_name, vtiger_potential.sales_stage, vtiger_groups.groupname FROM vtiger_potential
						...
						LEFT JOIN vtiger_groups ON vtiger_groups.groupid=vtiger_crmentity.assigned_user_id' . Users_Privileges_Model::getNonAdminAccessControlQuery(
                $this->getName()
            ) . '
						INNER JOIN vtiger_sales_stage ON vtiger_potential.sales_stage =  vtiger_sales_stage.sales_stage 
    ...
    function getTotalRevenuePerSalesPerson($dateFilter)
    {
        ...
        $result = $db->pquery(
            'SELECT sum(amount) amount, vtiger_users.userlabel as last_name,vtiger_users.id as id,DATE_FORMAT(closingdate, "%d-%m-%Y") AS closingdate  FROM vtiger_potential
						INNER JOIN vtiger_crmentity ON vtiger_potential.potentialid = vtiger_crmentity.crmid
						INNER JOIN vtiger_users ON vtiger_users.id=vtiger_crmentity.assigned_user_id AND vtiger_users.status="ACTIVE"
						AND vtiger_crmentity.deleted = 0 ' . Users_Privileges_Model::getNonAdminAccessControlQuery(
                $this->getName()
            ) . 'WHERE sales_stage = ? ' . ' ' . $dateFilterSql . ' GROUP BY assigned_user_id',
            $params
        );
        $data = [];
        for ($i = 0; $i < $db->num_rows($result); $i++) {
