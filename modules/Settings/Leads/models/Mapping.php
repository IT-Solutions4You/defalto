...
class Settings_Leads_Mapping_Model extends Settings_Vtiger_Module_Model
{
    ...
    public function getFieldsInfo($fieldIdsList)
    {
        ...
        $db = PearDatabase::getInstance();
        $result = $db->pquery(
            'SELECT fieldid, fieldlabel, uitype, typeofdata, fieldname, tablename, tabid FROM vtiger_field WHERE fieldid IN (' . generateQuestionMarks($fieldIdsList) . ')',
            $fieldIdsList
        );
        $numOfRows = $db->num_rows($result);
    ...
    public function getFieldsInfoByName(array $fieldNamesList): array
    {
        ...
        $db = PearDatabase::getInstance();
        $result = $db->pquery(
            'SELECT fieldid, fieldlabel, uitype, typeofdata, fieldname, tablename, tabid FROM vtiger_field WHERE fieldname IN (' . generateQuestionMarks($fieldNamesList) . ')',
            $fieldNamesList
        );
        $numOfRows = $db->num_rows($result);
