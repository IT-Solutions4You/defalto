...
class Settings_CustomerPortal_Module_Model extends Settings_Vtiger_Module_Model
{
    ...
    public function getContactRelatedModulesList()
    {
        ...
        $restrictedModules = ['ModComments', 'Potentials', 'PurchaseOrder', 'SalesOrder', 'Campaigns', 'Vendors'];
    ...
    public function save()
    {
        ...
        if ($dashboardWidgets) {
            $db->pquery(
                'UPDATE vtiger_customerportal_settings SET default_assignee=?, support_notification=?, announcement=?, widgets=?',
                [$defaultAssignee, $renewalPeriod, $announcement, $dashboardWidgets]
            );
        }
        ...
        if (!empty($relatedModuleList)) {
            foreach ($relatedModuleList as $module => $info) {
                $tabid = getTabid($module);
                $db->pquery(
                    'INSERT INTO vtiger_customerportal_relatedmoduleinfo(tabid, relatedmodules) VALUES(?,?) ON DUPLICATE KEY UPDATE relatedmodules = ?',
                    [$tabid, $info, $info]
                );
            }
        }
        ...
        if (!empty($recordPermissions)) {
            ...
            $db->pquery(
                ...
                [$updatedPermissions['create'], $updatedPermissions['edit'], getTabid($updatedPermissions['module'])]
            );
        }
    }
    ...
    public static function isFieldCustomerPortalEditable($crmStatus, $value, $module)
    {
        ...
        if ($crmStatus && $value->name !== 'assigned_user_id' && $value->name !== 'contact_id') {
            ...
            switch ($module) {
                case 'HelpDesk'    :
                    if (in_array($value->name, ['contact_id', 'parent_id'])) {
                        $isFieldEditable = 0;
                    }
                    break;
                case 'Assets'    :
                    if (in_array($value->name, ['account', 'contact', 'datesold', 'serialnumber'])) {
                        $isFieldEditable = 0;
                    }
                    break;
            }
