...
class Vtiger_Utils
{
    ...
    static function TableHasForeignKey($tablename, $key)
    {
        ...
        $tablename = Vtiger_Util_Helper::validateStringForSql($tablename);
        $rs = $db->pquery(
            "SELECT 1 FROM information_schema.TABLE_CONSTRAINTS WHERE CONSTRAINT_TYPE = 'FOREIGN KEY' AND TABLE_SCHEMA = ? AND TABLE_NAME = ? AND CONSTRAINT_NAME = ?",
            [$db->dbName, $tablename, $key]
        );
