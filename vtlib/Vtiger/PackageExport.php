...
class Vtiger_PackageExport
{
    ...
    function export_Actions($moduleInstance)
    {
        ...
        global $adb;
        $result = $adb->pquery(
            'SELECT distinct(actionname) FROM vtiger_profile2utility, vtiger_actionmapping
			WHERE vtiger_profile2utility.activityid=vtiger_actionmapping.actionid and tabid=?',
            [$moduleInstance->id]
        );
