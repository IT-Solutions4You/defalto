...
class Vtiger_Field extends Vtiger_FieldBasic
{
    ...
    function setPicklistValues($values)
    {
        ...
        foreach ($values as $value) {
            ...
            if (is_array($value)) {
                ...
                $adb->pquery(
                    "INSERT INTO $picklist_table($picklist_idcol, $this->name, presence, picklist_valueid,sortorderid) VALUES(?,?,?,?,?)",
                    [$new_id, $value, $presence, $newPicklistValueId, $sortOrderId]
                );
            }

            // Associate picklist values to all the role
            $adb->pquery(
                "INSERT INTO vtiger_role2picklist(roleid, picklistvalueid, picklistid, sortid) SELECT roleid, $newPicklistValueId, $newPicklistId, $sortOrderId FROM vtiger_role",
                []
            );
        }
    }
    ...
    function setNoRolePicklistValues($values)
    {
        ...
        foreach ($values as $value) {
            ...
            if (is_array($value)) {
                $adb->pquery(
                    "INSERT INTO $picklist_table($picklist_idcol, $this->name, sortorderid, presence, color) VALUES(?,?,?,?,?)",
                    [$new_id, $value[0], $sortid, $presence, $value[1]]
                );
            } else {
