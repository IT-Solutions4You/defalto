...
class CRMEntity extends CRMExtension
{
    ...
    function insertIntoCrmEntity($module, $fileid = '')
    {
        ...
        if ($this->mode == 'edit') {
            ...
            if ($acl->is_admin == true || $acl->profileGlobalPermission[1] == 0 || $acl->profileGlobalPermission[2] == 0 || $this->isWorkFlowFieldUpdate) {
                ...
                $perm_qry = "SELECT columnname FROM vtiger_field INNER JOIN vtiger_profile2field ON vtiger_profile2field.fieldid = vtiger_field.fieldid WHERE vtiger_field.tabid = ? AND vtiger_profile2field.visible = 0 AND vtiger_profile2field.readonly = 0 AND vtiger_profile2field.profileid IN (" . generateQuestionMarks(
                        $profileList
        ...
        if ($insertion_mode == 'edit') {
            ...
            if ($acl->is_admin == true || $acl->profileGlobalPermission[1] == 0 || $acl->profileGlobalPermission[2] == 0 || $this->isWorkFlowFieldUpdate) {
                $sql = "select fieldname,columnname,uitype,generatedtype,
										typeofdata from vtiger_field where tabid in (" . generateQuestionMarks(
                        $tabid
                    ) . ") and tablename=? and displaytype in (1,3,6) and presence in (0,2) group by columnname";
                        ...
                        if ($_REQUEST['action'] == 'MassSave' || $_REQUEST['action'] == 'MassEditSave') {
                            if ($IMG_FILES[0]['error'] == 0) {
                                ...
                                if (php7_count($oldImageAttachmentIds)) {
                                    $adb->pquery(
                                        'DELETE FROM vtiger_seattachmentsrel WHERE attachmentsid IN (' . generateQuestionMarks($oldImageAttachmentIds) . ')',
                                        $oldImageAttachmentIds
                                    );
                                    $adb->pquery(
                                        'DELETE FROM vtiger_attachments WHERE attachmentsid IN (' . generateQuestionMarks($oldImageAttachmentIds) . ')',
                                        $oldImageAttachmentIds
                                    );
    ...
    function retrieve_entity_info($record, $module, $allowDeleted = false)
    {
        ...
        if ($cachedModuleFields) {
            ...
            if (!$result || $adb->num_rows($result) < 1) {
                ...
                $this->column_fields['label'] = !empty($resultrow['label']) ? $resultrow['label'] : Vtiger_Record_Model::generateLabel(
                    $module,
                    $this->column_fields->getColumnFields()
                );
            }
        }
    ...
    function process_list_query($query, $row_offset, $limit = -1, $max_per_page = -1)
    {
        ...
        if ($rows_found != 0) {
            ...
            for (
                $index = $row_offset, $row = $this->db->fetchByAssoc(
                $result,
                $index
            ); $row && ($index < $row_offset + $max_per_page || $max_per_page == -99); $index++, $row = $this->db->fetchByAssoc($result, $index)
            ) {
...
//Added by Richie
    ...
    function unlinkDependencies($module, $id)
    {
        ...
        $fieldRes = $this->db->pquery(
            'SELECT tabid, tablename, columnname FROM vtiger_field WHERE fieldid IN (
    ...
    function checkModuleSeqNumber($table, $column, $no)
    {
        ...
        $result = $adb->pquery(
            "select " . $adb->sql_escape_string($column) .
    ...
    function updateMissingSeqNumber($module)
    {
        ...
        if ($fieldinfo && $adb->num_rows($fieldinfo)) {
            ...
            if ($fld_table == $this->table_name) {
                $records = $adb->pquery(
                    "SELECT $this->table_index AS recordid FROM $this->table_name " .
    ...
    function isViewed($crmid = false)
    {
        ...
        if ($crmid) {
            global $adb;
            $result = $adb->pquery("SELECT viewedtime,modifiedtime,creator_user_id,assigned_user_id,modifiedby FROM vtiger_crmentity WHERE crmid=?", [$crmid]);
    ...
    {
        global $adb;
        $adb->pquery("UPDATE vtiger_crmentity set viewedtime=? WHERE crmid=? AND assigned_user_id=?", [date('Y-m-d H:i:s', time()), $this->id, $userid]);
    }
     ...
     * @param String $with_module Related module name
     * @param mixed $with_crmid Integer or Array of related module record number
     * @throws Exception
     */
    public function save_related_module($module, $crmid, $with_module, $with_crmid)
    {
        ...
        foreach ($with_crmid as $relcrmid) {
            $result = Core_Relation_Model::saveEntityRelation($crmid, $module, $relcrmid, $with_module);
    ...
    public function delete_related_module($module, $crmid, $with_module, $with_crmid)
    {
        ...
        foreach ($with_crmid as $relcrmid) {
            Core_Relation_Model::deleteEntityRelation($crmid, $module, $relcrmid, $with_module);
        }
    }
    ...
    public function get_inventory_list($id, $cur_tab_id, $rel_tab_id, $actions = false)
    {
        global $currentModule, $app_strings, $singlepane_view;
        ...
        $query .= " FROM $other->table_name";
        $query .= " INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = $other->table_name.$other->table_index";
        $query .= " INNER JOIN df_inventoryitem ON (df_inventoryitem.productid = vtiger_crmentity.crmid OR df_inventoryitem.parentid = vtiger_crmentity.crmid)";
        $query .= $more_relation;
        $query .= " LEFT  JOIN vtiger_users ON vtiger_users.id = vtiger_crmentity.assigned_user_id";
        $query .= " LEFT  JOIN vtiger_groups ON vtiger_groups.groupid = vtiger_crmentity.assigned_user_id";
        $query .= " WHERE vtiger_crmentity.deleted = 0 AND (df_inventoryitem.productid = $id OR df_inventoryitem.parentid = $id)";
        if ($related_module == 'Leads') {
            $query .= " AND vtiger_leaddetails.converted=0 ";
        }
    ...
    public function get_dependents_list($id, $cur_tab_id, $rel_tab_id, $relationId)
    {
        ...
        if (empty($relationFieldId)) {
            $dependentFieldSql = $this->db->pquery(
                "SELECT tabid, fieldname, columnname,tablename FROM vtiger_field WHERE uitype=10 AND fieldid IN (SELECT fieldid FROM vtiger_fieldmodulerel WHERE relmodule=? AND module=?)",
                [$currentModule, $related_module]
            );
        } else {
            $dependentFieldSql = $this->db->pquery(
                "SELECT tabid, fieldname, columnname,tablename FROM vtiger_field WHERE uitype=10 AND fieldid IN (SELECT relationfieldid FROM vtiger_relatedlists WHERE relation_id=?)",
                [$relationId]
            );
        }

        if (!$this->db->num_rows($dependentFieldSql)) {
            throw new Exception('Invalid Dependent List missing UIType 10 fields');
        }
        ...
        foreach ($transferEntityIds as $transferId) {
            ...
            for ($i = 0; $i < $numOfRecords; $i++) {
                ...
                $adb->pquery(
                    ...
                    [$entityId, $relcrmid, $relmodule, $transferId, $module]
                );
            }
            ...
            $parentRecords = $adb->pquery(
                "SELECT crmid, module FROM vtiger_crmentityrel WHERE relcrmid=? AND relmodule=?" .
            ...
            for ($i = 0; $i < $numOfRecords; $i++) {
                ...
                $adb->pquery(
                    ...
                    [$entityId, $parcrmid, $parmodule, $transferId, $module]
                );
            }
        }
