...
function vtws_addWebserviceOperation($name, $handlerFilePath, $handlerMethodName, $requestType, $preLogin = 0)
{
    ...
    $result = $adb->pquery($createOperationQuery, [
        $operationId,
        $name,
        $handlerFilePath,
        $handlerMethodName,
...
/**
 * This function is used to upload the company logo file. It creates the .ico file as the copy of the uploaded file.
 *
 ...
 */
function vtws_CreateCompanyLogoFile(string $fieldName): mixed
{
...
function vtws_transferOwnership($ownerId, $newOwnerId, $delete = true)
{
    ...
    $db->pquery(
        'UPDATE vtiger_crmentity SET assigned_user_id=?, modifiedtime = ? WHERE assigned_user_id=? AND setype<>?',
        [$newOwnerId, date('Y-m-d H:i:s'), $ownerId, 'ModComments']
    );
    ...
    $result = $db->pquery($sql, [52, 53, 77, 101, 10, 'Users', 'creator_user_id']);
    ...
    $db->pquery(
        'UPDATE vtiger_customview SET userid = ? WHERE userid = ? AND status = ? AND cvid IN (SELECT cvid FROM vtiger_cv2users UNION SELECT cvid FROM vtiger_cv2group UNION SELECT cvid FROM vtiger_cv2role UNION SELECT cvid FROM vtiger_cv2rs)',
        [$newOwnerId, $ownerId, 1]
    );
