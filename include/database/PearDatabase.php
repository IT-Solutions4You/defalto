...
$log = Vtiger_Logger_Helper::getLogger('VT');
$logsqltm = Vtiger_Logger_Helper::getLogger('SQLTIME');
...
class PearDatabase
{
    ...
    {
        $log1 = Vtiger_Logger_Helper::getLogger('VT');
        if (is_array($msg)) {
    ...
    function __construct($dbtype = '', $host = '', $dbname = '', $username = '', $passwd = '')
    {
        ...
        $this->log = Vtiger_Logger_Helper::getLogger('PearDatabase_' . $currentModule);
    ...
    function formatString($tablename, $fldname, $str)
    {
        ...
        foreach ($adoflds as $fld) {
            if (strcasecmp($fld->name, $fldname) == 0) {
                ...
                if (strcmp($fldtype, 'CHAR') == 0 || strcmp($fldtype, 'VARCHAR') == 0 || strcmp($fldtype, 'VARCHAR2') == 0 || strcmp($fldtype, 'LONGTEXT') == 0 || strcmp(
                        $fldtype,
                        'TEXT'
                    ) == 0) {
