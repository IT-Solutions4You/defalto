...
class QueryGenerator
{
        ...
        foreach ($this->conditionals as $index => $conditionInfo) {
            ...
            foreach ($valueSqlList as $valueSql) {
                if (in_array($fieldName, $this->referenceFieldList)) {
                    ...
                    if ($fieldName == 'created_user_id') {
                        $concatSql = getSqlForNameInDisplayFormat(['first_name' => "vtiger_users$fieldName.first_name", 'last_name' => "vtiger_users$fieldName.last_name"],
                            'Users');
                ...
                if (($conditionInfo['operator'] == 'n' || $conditionInfo['operator'] == 'k') && ($field->getFieldDataType() == 'owner' || $field->getFieldDataType(
                        ) == 'picklist')) {
                    $fieldGlue = ' AND';
                } else {
                    $fieldGlue = ' OR';
    ...
    protected function getConditionValue($value, $operator, $field)
    {
        ...
        if (is_string($value) && $this->ignoreComma == false) {
            ...
            if (in_array($operator, ['s', 'ew', 'c', 'k']) || in_array($field->getFieldDataType(), $commaSeparatedFieldTypes) || ($field->getFieldName(
                    ) == 'salutationtype') || ($field->getFieldDataType() == 'reference' && in_array('Users', $field->getReferenceList()))) {
    ...
    public function addRelatedModuleCondition($relatedModule, $column, $value, $SQLOperator)
    {
        $conditionNumber = $this->conditionInstanceCount++;
        $this->groupInfo .= "$conditionNumber ";
        $this->manyToManyRelatedModuleConditions[$conditionNumber] = [
            'relatedModule' =>
                $relatedModule,
            'column'        => $column,
            'value'         => $value,
    ...
    public function addReferenceModuleFieldCondition($relatedModule, $referenceField, $fieldName, $value, $SQLOperator, $glue = null)
    {
        ...
        $this->groupInfo .= "$conditionNumber ";
        $this->referenceModuleField[$conditionNumber] = [
            ...
            if (isset($input['search_text']) && $input['search_text'] != "") {
                ...
                if ($type == 'picklist') {
                    ...
                    if (empty($mod_strings)) {
                        throw new Exception('Missing current language strings');
                    }
    ...
    public function getDashBoardConditionList()
    {
        ...
        if (!empty($purchaseOrderId)) {
            $relatedConditionList[] = [
                'relatedModule'   => 'PurchaseOrder',
                'conditionModule' =>
                    'PurchaseOrder',
                'finalValue'      => $purchaseOrderId,
                'SQLOperator'     => '='
            ];
        }
