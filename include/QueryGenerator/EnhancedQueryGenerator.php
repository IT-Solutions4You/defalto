                ...
                if ($fieldType == 'reference') {
                    ...
                    foreach ($moduleList as $module) {
                        if ($module == 'Users' && $baseModule != 'Users') {
                            ...
                            $tableJoinCondition[$fieldName]['vtiger_currency_info' . $fieldName] = $fieldTable . '.' . $field->getColumnName(
                                ) . ' = vtiger_currency_info' . $fieldName . '.id';
                        } else {
                ...
                if ($fieldType == 'owner') {
                    ...
                    foreach ($moduleList as $module) {
                        if ($module == 'Users' && $baseModule != 'Users') {// && ($fieldName == 'created_user_id' || $fieldName == 'modifiedby')) {
                            ...
                            $tableJoinCondition[$fieldName]['vtiger_currency_info' . $referenceParentFieldName . $fieldName] = $fieldTable . $referenceParentFieldName . '.' . $field->getColumnName(
                                ) . ' = vtiger_currency_info' . $referenceParentFieldName . $fieldName . '.id';
                        } else {
                            if (!array_key_exists($fieldTable . $referenceParentFieldName, $tableJoinMapping)) {
                                $tableJoinMapping[$fieldTable . $referenceParentFieldName] = 'LEFT JOIN ' . $fieldTable . ' AS ';
                                $tableJoinCondition[$referenceParentFieldName . $fieldName][$fieldTable . $referenceParentFieldName] = $fieldTable . $referenceParentFieldName . '.' . $referenceModuleColumnIndex[$fieldTable] . ' = ' .
            ...
            foreach ($valueSqlList as $valueSql) {
                if (in_array($baseFieldName, $this->referenceFieldList)) {
                    ...
                    $concatSql = getSqlForNameInDisplayFormat([
                        'first_name' => 'vtiger_users' . $ownerTableName . '.first_name',
                        'last_name'  => 'vtiger_users' . $ownerTableName . '.last_name'
                    ], 'Users');
                ...
                } elseif (Vtiger_Functions::isUserSpecificFieldTable(
                        $field->getTableName(),
                        getTabModuleName($field->getTabId())
                    ) && $fieldName == "starred" && $conditionInfo['value'] != 1) {
    ...
    function getOrderByColumn($fieldName)
    {
        ...
        if ($orderByFieldModel && $orderByFieldModel->getFieldDataType() == 'reference') {
            ...
            if (in_array('DocumentFolders', $referenceModules)) {
                ...
                $columnSqlTable = 'vtiger_users' . $parentReferenceField . $fieldName;
                $orderByColumn = getSqlForNameInDisplayFormat([
                    'first_name' => $columnSqlTable . '.first_name',
                    'last_name'  => $columnSqlTable . '.last_name'
                ], 'Users');
            } else {
