...
function get_group_array($add_blank = true, $status = "Active", $assigned_user = "", $private = "", $module = false)
{
    ...
    if ($group_array == null) {
        ...
        if ($private == 'private') {
            ...
            if (!empty($current_user_groups) && (php7_count($current_user_groups) != 0)) {
                $query .= " union select vtiger_groups.groupid as groupid,vtiger_groups.groupname as groupname from vtiger_groups inner join vtiger_group2rs on vtiger_groups.groupid=vtiger_group2rs.groupid where vtiger_group2rs.roleandsubid in (" . generateQuestionMarks(
                        $parent_roles
                    ) . ")";
...
function is_uitype($uitype, $reqtype)
{
    $ui_type_arr = [
        '_date_'       => [5, 6, 23, 70],
        '_picklist_'   => [15, 16, 52, 53, 54, 55, 59, 62, 63, 66, 68, 76, 77, 78, 80, 98, 101, 115, 357, 14001],
        '_users_list_' => [52],
    ];
...
function getRelationTables($module, $secmodule)
{
    ...
    if ($adb->num_rows($ui10_query) > 0) {
        ...
        if ($primary_obj->table_name == $ui10_tablename) {
            ...
            $reltables = [
                $ui10_tablename                    => ["$ui10_columnname", "" . $secondary_obj->table_index . ""],
...
function getDuplicatesPreventionMessage($moduleName, $duplicateRecordsList)
{
    ...
    for ($i = 0; $i < $duplicateRecordsCount && $i < 5; $i++) {
        ...
        $message .= '<a href="' . $dupliRecordModel->getDetailViewUrl() . '" target="_blank" style="color:#15c !important">' . Vtiger_Functions::getCRMRecordLabel(
                $dupliRecordId
            ) . '</a><br>';
    }
