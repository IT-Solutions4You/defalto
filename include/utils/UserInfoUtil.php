...
function isReadPermittedBySharing($module, $tabid, $actionid, $record_id)
{
    ...
    if (is_array($relatedModuleArray)) {
        foreach ($relatedModuleArray as $parModId) {
            ...
            if (php7_count($parRecordOwner) > 0) {
...
function getRoleInformation($roleid)
{
    ...
    $parentRoleArr = explode('::', $parentrole ? : '');
    $parentRoleArrLen = php7_count($parentRoleArr);
...
function getCombinedUserGlobalPermissions($userId)
{
    ...
    $no_of_profiles = php7_count($profArr);
    $userGlobalPerrArr = [];
...
function getListViewSecurityParameter($module)
{
    ...
    if ($module == 'Leads') {
        $sec_query .= " and (
            vtiger_crmentity.assigned_user_id in($current_user->id)
            or vtiger_crmentity.assigned_user_id in(select vtiger_user2role.userid from vtiger_user2role inner join vtiger_users on vtiger_users.id=vtiger_user2role.userid inner join vtiger_role on vtiger_role.roleid=vtiger_user2role.roleid where vtiger_role.parentrole like '" . $current_user_parent_role_seq . "::%')
            or vtiger_crmentity.assigned_user_id in(select shareduserid from vtiger_tmp_read_user_sharing_per where userid=" . $current_user->id . " and tabid=" . $tabid . ")
            or (";
        ...
        $sec_query .= " and (vtiger_crmentity.assigned_user_id in($current_user->id) 
            or vtiger_crmentity.assigned_user_id in(select vtiger_user2role.userid from vtiger_user2role inner join vtiger_users on vtiger_users.id=vtiger_user2role.userid inner join vtiger_role on vtiger_role.roleid=vtiger_user2role.roleid where vtiger_role.parentrole like '" . $current_user_parent_role_seq . "::%') 
            or vtiger_crmentity.assigned_user_id in(select shareduserid from vtiger_tmp_read_user_sharing_per where userid=" . $current_user->id . " and tabid=" . $tabid . ") or (";
    ...
    if ($is_admin || $profileGlobalPermission[1] == 0 || $profileGlobalPermission[2] == 0) {
        ...
        if (php7_count($profilelist) > 0) {
            if ($accessmode == 'readonly') {
                $query = "SELECT vtiger_profile2field.visible FROM vtiger_field INNER JOIN vtiger_profile2field ON vtiger_profile2field.fieldid=vtiger_field.fieldid WHERE vtiger_field.tabid=? AND vtiger_profile2field.visible=0 AND vtiger_profile2field.profileid in (" . generateQuestionMarks(
                        $profilelist
                    ) . ") AND vtiger_field.fieldname= ? and vtiger_field.presence in (0,2) GROUP BY vtiger_field.fieldid";
            } else {
                $query = "SELECT vtiger_profile2field.visible FROM vtiger_field INNER JOIN vtiger_profile2field ON vtiger_profile2field.fieldid=vtiger_field.fieldid WHERE vtiger_field.tabid=? AND vtiger_profile2field.visible=0 AND vtiger_profile2field.readonly=0 AND vtiger_profile2field.profileid in (" . generateQuestionMarks(
                        $profilelist
                    ) . ") AND vtiger_field.fieldname= ? and vtiger_field.presence in (0,2) GROUP BY vtiger_field.fieldid";
            }
